...
def detect_crop(slides_path: Path, offset: float = 0.0, sample_s: int = 60) -> CropRegion | None:
    ...
    result = subprocess.run(
        ["ffmpeg", "-y",
         ...
         "-t", str(sample_s),
         "-vf", "cropdetect=limit=10:round=2:skip=2",
         "-f", "null", "-"],
...
def _resolve_crop(args: argparse.Namespace, slides_path: Path, offset: float) -> CropRegion | None:
    """Return a CropRegion from manual args, autocrop, or both combined."""
    ...
    if args.slides_autocrop:
        region = detect_crop(slides_path, offset=offset)
        if region:
            # Apply manual adjustments on top of autocrop
            return CropRegion(
                x=region.x + left,
                y=region.y + top,
                w=region.w - left - right,
                h=region.h - top - bottom,
            )
